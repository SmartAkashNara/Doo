...
-(void)TPKeyboardAvoiding_scrollToActiveTextField {
    ...
    // kiran
    // state.animationDuration was setting directly. So when you will be switching between textfields manually, animation was not occuring at the screen and it was looking jurky. So added if animationDuration is 0.0, make 0.2 atleast.
    CGFloat animationDuration = state.animationDuration;
...
-(CGFloat)TPKeyboardAvoiding_idealOffsetForView:(UIView *)view withViewingAreaHeight:(CGFloat)viewAreaHeight {
    ...
    // Kiran
    NSInteger moreOffSet = view.tag;
    if (moreOffSet > 10 && moreOffSet < 200) {
        offset += moreOffSet;
    }
