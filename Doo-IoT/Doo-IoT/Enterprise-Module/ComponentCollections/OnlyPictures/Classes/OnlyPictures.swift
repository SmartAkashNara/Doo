...
private var INTERNALGAP_OF_COUNT: CGFloat = 20
...
@objc public protocol OnlyPicturesDelegate {
    ...
    @objc optional func pictureViewDidSelect(_ onlyPictureView: OnlyPictures)
}
...
internal extension OnlyPictures {
    ...
    @objc private func pictureTapActionListener(recognizer: UITapGestureRecognizer){
        ...
        // generic tap called
        self.delegate?.pictureViewDidSelect?(self)
    }
...
extension OnlyPictures {
    ...
    func setCountFlexibleWidthWith(_ count: Int){
        ...
        if let image = self.imageInPlaceOfCount {
            ...
            self.calculatedWidthOfCount = (width+INTERNALGAP_OF_COUNT)>SIZE_OF_IMAGEVIEWS ? (width+INTERNALGAP_OF_COUNT) : SIZE_OF_IMAGEVIEWS
